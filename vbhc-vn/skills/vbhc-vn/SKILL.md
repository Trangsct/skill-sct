...
- `reference/phong-tranh-sai-lam.md` — chi tiết 14 nhóm sai lầm A–N + checklist + vụ thật (nhóm "K" cũ về lắp ghép đã hợp nhất vào H = H6–H11; trHeight = H12; Nhóm J 31/8/2026 — giọng giải thích lọt vào thân văn bản; **Nhóm K 06/9/2026 (K1–K10, theo bản cuối Bạn chốt) — văn bản chỉ đạo UBND tỉnh: gần như không viện dẫn, điều kiện lên cơ quan khác chỉ khi Sở đã được giao vai trò + có sản phẩm, viết tổng quát cho ngành khác, không tính từ đánh giá, thuật ngữ theo luật**).
- `scripts/normalize_body.py` — **Chế độ B, chạy TRƯỚC và SAU khi sửa file cơ quan khác gửi đến**: gỡ `w:numPr` (danh sách tự động sinh thêm dấu "-" thành "- -"), xóa `w:ind` lẫn lộn rồi đặt lại lùi đầu dòng đồng nhất, gỡ `w:tab` đầu đoạn, xóa đoạn trống thừa giữa thân. `--check` chỉ đếm, `--indent` chọn 567 hoặc 720. Chi tiết: Nhóm N.
...
- `scripts/fit_pages.py` — **căn trang hợp đồng, phụ lục hợp đồng và văn bản dài bằng giãn dòng Exactly 17–21pt** (Quy tắc 28): `sweep` dò thử từng mức và in số trang, `apply <pt>` xuất bản đã chọn; tự gán keepNext/keepLines cho đề mục "Điều N." và keepNext cho các đoạn cuối trước bảng ký. KHÔNG dùng cho VBHC theo NĐ 30/2020.
...
- **Quốc hiệu "CỘNG HÒA XÃ HỘI CHỦ NGHĨA VIỆT NAM" (HÒA, không "HOÀ"); tiêu ngữ "Độc lập – Tự do – Hạnh phúc" dùng EN DASH "–"** (không gạch nối "-"). Nhiều mẫu thật trong `examples/` gốc còn sai — đã chuẩn hóa 02/9/2026; file người dùng tải lên thì chạy `python3 scripts/fix_quoc_hieu.py <file.docx>` ngay sau build (qa_all.py báo FAIL `QUOCHIEU` nếu còn). **Dòng "Kính gửi:" KHÔNG in đậm** (qa_all.py báo WARN `KINHGUI`).
...
- **D — Hiệu lực:** mọi VBQPPL viện dẫn phải còn hiệu lực **và ĐÃ có hiệu lực tại ngày ký** (không dẫn NQ/NĐ chưa đến ngày hiệu lực, không đưa nội dung không liên quan trực tiếp — vụ NQ 66.25 ngày 11/9/2026); tra trạng thái với VB ban hành trước 2024; đối chiếu danh mục hết hiệu lực theo **NĐ 79/2025** (xem `reference/nd-79-2025-tom-tat.md`). Bảng VB đã thay thế ở file reference.
...
20. **Không đặt chiều cao dòng cố định `<w:trHeight>` cho bảng nội dung** — trHeight + cantSplit đẩy nguyên bảng sang trang sau, trang trước trắng nửa trang (vụ 24/7/2026). Chế độ A không ghi; Chế độ B gỡ sạch (trừ bảng header; chừa chỗ ký tay dùng paragraph trống trong ô — đây cũng là ngoại lệ duy nhất của Quy tắc 3). QA: `check_document.py` nhóm [F]. *(Chi tiết + lệnh gỡ nhanh + mốc 6cm: Nhóm H12.)*
...
27. **KHỐI KÍNH GỬI NHIỀU NƠI VÀ BẢN XUẤT BẢN KHÔNG CHỖ TRỐNG (Bạn chốt 16/9/2026):** (a) "Kính gửi:" và tên các cơ quan **thẳng cột** (tab + hanging indent — ngoại lệ có chủ đích duy nhất của quy tắc không thụt treo), **cả khối cân giữa trang**, mỗi cơ quan đúng 1 dòng — dòng dài thì giảm lùi (2 500–3 760 dxa) hoặc chia lại nội dung, không để gãy 3 dòng; (b) **mọi đoạn thân, kể cả đề mục và gạch đầu dòng, lùi 1,27 cm đồng đều** — Bạn bác bản "lùi tiến không đều"; (c) file Bạn yêu cầu **"hoàn thiện để xuất bản": cấm còn "……", "…/…/2026", "(nêu số liệu)", chữ tím** — căn cứ chưa có số thì viết thẳng tên văn bản ("Thực hiện Thông báo kết luận giám sát của Ban Thường vụ…"), kết quả chưa có số thì viết định tính thành câu hoàn chỉnh; chỉ dòng ngày tháng góc phải được để trống bằng khoảng trắng cho văn bản ký sau.

28. **CĂN TRANG BẰNG GIÃN DÒNG EXACTLY 17–21PT CHO HỢP ĐỒNG, PHỤ LỤC HỢP ĐỒNG VÀ VĂN BẢN DÀI (Bạn chốt 17/9/2026, bộ 04 hợp đồng tư vấn CCN Mông Sơn và KCN Minh Quân mở rộng):** (a) **phạm vi**: hợp đồng kinh tế, phụ lục hợp đồng, văn bản dài nhiều trang — **VBHC theo NĐ 30/2020 và báo cáo định kỳ của Phòng giữ giãn dòng đơn, không Exactly**; (b) đặt `w:line` 340–420 dxa kèm `w:lineRule="exact"` (17–21pt); **dưới 17pt dấu tiếng Việt bị cắt khi in**, trên 21pt trang rỗng và dôi trang; (c) **cách chọn mức**: chạy `scripts/fit_pages.py sweep <file>.docx` để dò 17 / 17,5 / 18 / 19 / 20 / 21pt, chọn mức cho số trang nhỏ nhất mà **trang cuối vẫn đủ nội dung** — cấm để trang cuối chỉ có mỗi khối ký; mỗi hợp đồng trong một bộ được chọn mức riêng, không ép cùng một mức; (d) kèm theo: đề mục "Điều N." gán `keepNext` + `keepLines`, 2–3 đoạn cuối trước bảng ký gán `keepNext`, hàng ký `cantSplit` — điều khoản hiệu lực và câu "Hợp đồng gồm … trang" phải nằm cùng trang với khối ký; (e) **sau khi chốt mức phải sửa lại câu "Hợp đồng gồm … trang" theo số trang PDF thật** (đổi giãn dòng mà quên sửa số trang là lỗi giao bản); (f) QA: render PDF, soi ảnh trang đầu và trang cuối; kiểm dấu tiếng Việt (ộ, ề, ế) không bị cắt ngọn ở mức đã chọn. *Vụ thật: bộ 4 hợp đồng ngày 17/9/2026 — QHCT Mông Sơn 19pt (7 trang + 1 phụ lục), BCNCKT Mông Sơn 18pt (7 trang), TKBVTC Mông Sơn 17,5pt (7 trang), BCNCKT Minh Quân mở rộng 17,5pt (7 trang).*
